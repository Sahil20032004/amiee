...
import { deleteRestaurant, getAllRestaurants, uploadRestaurantPicture } from '../redux/actions/restaurantAction';
...
const AllRestaurants = () => {
    ...
    const { isLoading, allRestaurants } = useSelector((state) => state.restaurantState);
    const [open, setOpen] = useState(false);
    const [isDeleteOpen, setIsDeleteOpen] = useState(false);
    const [restaurantRef, setRestaurantRef] = useState(undefined);
    const [images, setImages] = useState([]);
    const [page, setPage] = useState(0);
    const rowsPerPage = 5;
    const emptyRows = Math.max(0, (1 + page) * rowsPerPage - allRestaurants?.length);
    ...
    const updloadImageHandler = () => {
        ...
        dispatch(uploadRestaurantPicture(formData, restaurantRef._id))
        setOpen(!open);
        setImages([]);
        setRestaurantRef(undefined);
    }

    const deleteHandler = () => {
        dispatch(deleteRestaurant(restaurantRef._id));
    ...
    return (
        <Fragment>
            <Meta title="All Restaurants" />
            <div className="flex">
                <SideBar />
                <Fragment>
                    {isLoading ? <Loader /> : (
                        <div className="w-[80%] sm:w-[60%] md:w-[70%] mx-auto mt-3">
                            <h2 className="text-2xl font-medium text-center my-8">All Restaurants</h2>
                            <TableContainer component={Paper}>
                                <Table className="min-w-[700px]">
                                    ...
                                    <TableBody>
                                        {(rowsPerPage > 2 ? allRestaurants?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : allRestaurants)?.map((restaurant) => (
                                            <TableRow key={restaurant._id} style={{ height: 72.8 }}>
                                                ...
                                                <TableCell align="center">
                                                    <Link to={`/admin/restaurant/${restaurant._id}/update`}>
                            ...
                            <Dialog open={open} scroll="body" className="!w-screen" fullWidth={true}>
                                <div className="p-4" >
                                    <DialogTitle className="text-center">Upload Restaurant Images</DialogTitle>
                            ...
                            <Dialog open={isDeleteOpen}>
                                ...
                                <DialogContent className="m-8">
                                    <DialogContentText className="text-gray-900">This will delete restaurant's room and its booking details also.</DialogContentText>
                                </DialogContent>
