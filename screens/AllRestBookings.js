...
import { changeBookingStatus, getAllBookings } from '../redux/actions/restaurantAction';
...
const AllRestBookings = () => {
    ...
    return (
        <Fragment>
            <Meta title="All Restaurant Bookings" />
            <div className="flex">
                <SideBar />
                <Fragment>
                    {isLoading ? <Loader /> : (
                        <div className="w-[80%] sm:w-[60%] md:w-[70%] mx-auto mt-3">
                            <h2 className="text-2xl font-medium text-center my-8">All Restaurant Bookings</h2>
                                    ...
                                    <TableBody>
                                        {(rowsPerPage > 2 ? allBookings?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : allBookings)?.map((booking) => (
                                            <TableRow key={booking._id} style={{ height: 72.8 }}>
                                                ...
                                                <TableCell align="center">
                                                    <Link to={`/admin/restaurantbooking/${booking._id}`}>
                            ...
                            <Dialog open={open}>
                                <div className="p-5">
                                    ...
                                    <DialogActions className="mt-4">
                                        <button onClick={() => setOpen(!open)} className="bg-orange-400 hover:bg-orange-500 py-2 rounded-lg w-24 text-center text-neutral-50  transition duration-200 font-semibold">Cancel</button>
                                        <button
                                            disabled={status === bookingRef?.status ? true : false}
                                            onClick={editHandler} className=" border-orange-400 text-orange-400 hover:text-orange-500 hover:border-orange-500 hover:bg-orange-200 border-solid border py-2 rounded-lg w-24 text-center transition duration-200 box-border">Update</button>
                                    </DialogActions>
                                </div>
                            </Dialog>
                        </div>
...
export default AllRestBookings;